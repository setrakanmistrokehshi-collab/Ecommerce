...
// ── PUBLIC: Get all active categories (storefront) ────────────────
...
// ─────────────────────────────────────────────────────────────────
router.use('/admin', protect, restrictTo(...STAFF_ROLES), auditLog);
    ...
    try {
      ...
      if (productCount > 0) {
        return next(new AppError(
          `Cannot delete category with ${productCount} active products. Move or delete products first.`,
          400
        ));
      }
...
// ⚠️ This MUST stay last — it catches anything that didn't match above
